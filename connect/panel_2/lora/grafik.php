<?php
	...
	$tanggal = mysqli_query($koneksi, "SELECT tanggal FROM panel1_lora ORDER BY id DESC LIMIT 0, 5");

	//membaca informasi suhu pada database sumbu y
	$tegangan = mysqli_query($koneksi, "SELECT tegangan FROM panel1_lora ORDER BY id DESC");

	//membaca informasi ldr pada database sumbu y
	$arus = mysqli_query($koneksi, "SELECT arus FROM panel1_lora ORDER BY id DESC");

	//membaca informasi kelembaban pada database sumbu y
	$daya = mysqli_query($koneksi, "SELECT daya FROM panel1_lora ORDER BY id DESC" );

	// membaca informasi tegangan pada database sumbu y
	$lux = mysqli_query($koneksi, "SELECT lux FROM panel1_lora ORDER BY id DESC");
...
<div class="panel-body">
	<canvas id="panel1_lora"></canvas>

	<!-- gambar grafik -->
	<script type="text/javascript">
		//baca is canvas
		var canvas = document.getElementById('panel1_lora');
